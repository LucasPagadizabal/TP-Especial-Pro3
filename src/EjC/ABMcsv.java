		...
		try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
			...
			while ((line = br.readLine()) != null) {
				...
				if(saltoLinea){//este if es para saltear el encabezado DNI-Gusto...
					...
					((ListaVinculada) l).addFinal(items);
	...
	public void insert (Lista l){
		...
		try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
			...
			while ((line = br.readLine()) != null) {
				...
				String contenidoLinea1 = items[0]+";"+(( time_end - time_start )/1000);
	...
	public void busqueda(Lista l){
		...
		try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
			...
			while ((line = br.readLine()) != null) {
				...
				if(existe){
					 contenidoLinea = items[0]+";"+(( time_end - time_start )/1000)+";Existe";
				}else{
					 contenidoLinea = items[0]+";"+(( time_end - time_start )/1000)+";NoExiste";
				}
